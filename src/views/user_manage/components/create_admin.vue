        ...
        <div class="dialog-content">
            <el-form :model="AdminFormData" :rules="rules" label-width="100px">
                ...
                <el-form-item label="密码" prop="password">
                    <el-input v-model="AdminFormData.password" show-password />
                </el-form-item>
...
bus.on('createId', (id: number) => {
    if (id == 1) {
        title.value = '新建产品管理员'
        AdminFormData.identity = '产品管理员'
    }
    if (id == 2) {
        title.value = '新建用户管理员'
        AdminFormData.identity = '用户管理员'
    }
    if (id == 3) {
        title.value = '新建消息管理员'
        AdminFormData.identity = '消息管理员'
    }
})
...
const addAdmin = async () => {
    const res = await createAdmin(AdminFormData)
