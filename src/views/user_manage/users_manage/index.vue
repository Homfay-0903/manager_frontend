...
const searchAdmin = async () => {
    tableData.value = (await searchUser(adminAccount.value, '用户管理员')).data
}
...
const returnAdminListLength = async () => {
    const res = (await getAdminListLength('用户管理员')).data
...
const getFirstPageList = async () => {
    tableData.value = (await returnListData(1, '用户管理员')).data
}
getFirstPageList()

const currentChange = async (value: number) => {
    paginationData.currentPage = value
    tableData.value = (await returnListData(paginationData.currentPage, '用户管理员')).data
}
