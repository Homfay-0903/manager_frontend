<template>
    <el-dialog v-model="dialogFormVisible" title='添加产品入库' width="600px" align-center draggable>
        <div class="dialog-content">
            <el-form :model="formData" :label-position="labelPosition" :rules="rules" label-width="120px">
        ...
        <template #footer>
            <span class="dialog-footer" @click="addProduct">
...
const addProduct = async () => {
    const res = (await createProduct(formData)).data
    if (res.status == 0) {
        ElMessage({
            message: '产品入库成功',
            type: 'success',
