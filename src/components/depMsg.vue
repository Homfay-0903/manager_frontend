...
const getUserDepartmentMessage = async () => {
    ...
    if (department !== null) {
        ...
        //console.log(res)
        //console.log(res.read_list)
        tableData.value = (await getDepartmentMsgList(department)).data
        if (res[0].read_status == 0) {
            const {
                read_list
            } = (await getDepartmentMsg(id, department)).data
...
const idInList = (id: number) => {
    ...
    return readList.value.includes(id) ? 0 : 1
